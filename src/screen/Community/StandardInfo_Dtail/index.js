...
export default function StandardInfo_Dtl() {
  ...
  const handleDeleteOption = async (codeGroup, optionKey, optionValue,targetCode) => {
    if (!window.confirm(`정말로 옵션 [${optionKey}]을 삭제하시겠습니까?`)) return;
  
    try {
      const res = await fetch('https://crm.skynet.re.kr/request/deleteStdOption', {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
      ...
      await res.json();
      alert('옵션이 삭제되었습니다.');
      getStdOptions(codeGroup, stdValue); // 옵션 리스트 갱신
    } catch (error) {
      alert('Error: ' + error);
    }
  };

  const handleUpdateOption = async (option, updatedOptionKey, updatedOptionValue, updatedOptionDesc) => {
    try {
      const res = await fetch('https://crm.skynet.re.kr/request/updateStdOption', {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          option_key: updatedOptionKey || option.code_option,
          option_value: updatedOptionValue || option.code_value,
          option_desc: updatedOptionDesc || option.code_desc,
          target_code: option.target_code,
        }),
      });
  
      if (!res.ok) throw new Error(`Server responded with status code ${res.status}`);
      alert('옵션이 수정되었습니다.');
      getStdOptions(option.code_group, option.target_code); // 옵션 리스트 갱신
    } catch (error) {
      alert('Error: ' + error);
    }
  };
  ...
  const handleOrderChange = async (option, newOrder) => {
    // 클라이언트에서 순서 값을 즉시 변경
    setCodeOptionList(prevOptions =>
      prevOptions.map((item) =>
        item.code_option === option.code_option && item.code_value === option.code_value
          ? { ...item, code_idx: newOrder }
          : item
      )
    );
    ...
    // 서버에 순서 업데이트 요청
    try {
      const res = await fetch('https://crm.skynet.re.kr/request/updateStdOptionOrder', {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          code_group: option.code_group,
          option_key: option.code_option,
          option_value: option.code_value,
          code_idx: newOrder // 새로운 순서 값
        })
      });
              ...
              <tbody>
                  {codeOptionList && codeOptionList.map((option, index) => (
                    <tr key={index}>
                      ...
                      <input
                          type="number"
                          value={option.code_idx}
                          onChange={(e) => handleOrderChange(option, e.target.value)} // option 객체를 전달
                          style={{ width: '30px' }}
                        />
                      </td>
                      ...
                      <td>
                        <DeleteButton onClick={() => handleDeleteOption(option.code_group, option.code_option, option.code_value,option.target_code)}>
                          X
                        </DeleteButton>
                        <EditButton
                        onClick={() =>
                          handleUpdateOption(option, optionKey, optionValue, optionDesc)
                        }
                      >
                        수정
                      </EditButton>
