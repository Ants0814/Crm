...
export default function Product() {
  ...
  useEffect(() => {
    ...
    const fetchBoardTables = async () => {
      try {
        const response = await fetch('https://crm.skynet.re.kr/request/getBoardTables?category=Product',{credentials:'include'}); // 실제 API 엔드포인트로 변경
        const data = await response.json();

        // 가져온 테이블 이름을 링크로 변환
        const dynamicLinks = data.map(table => ({
          path: `/Product/Board/${table.boardId}`,
  ...
  return (
    <div className="Product" style={{ display: 'flex', width: '100%', height: '100%' }}>
