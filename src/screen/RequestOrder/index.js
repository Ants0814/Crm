...
export default function RequestOrder() {
  ...
  useEffect(() => {
    ...
    const fetchBoardTables = async () => {
      try {
        const response = await fetch('https://crm.skynet.re.kr/request/getBoardTables?category=RequestOrder',{credentials:'include'}); // 실제 API 엔드포인트로 변경
        const data = await response.json();

        // 가져온 테이블 이름을 링크로 변환
        const dynamicLinks = data.map(table => ({
          path: `/RequestOrder/Board/${table.boardId}`,
  ...
  return (
    <div className="RequestOrder" style={{ display: 'flex', width: '100%', height: '100%' }}>
