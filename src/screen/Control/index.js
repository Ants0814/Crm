...
const staticLinks = [
  // { path: '/Control/time-control', label: '시간컨트롤' },
  // { path: '/Control/time-control', label: '팝업컨트롤' },
  // { path: '/Control/time-control', label: '그룹별극장컨트롤' },
];
...
export default function Control() {
  ...
  useEffect(() => {
    ...
    const fetchBoardTables = async () => {
      try {
        const response = await fetch('https://crm.skynet.re.kr/request/getBoardTables?category=Control',{credentials:'include'}); // 실제 API 엔드포인트로 변경
        const data = await response.json();

        // 가져온 테이블 이름을 링크로 변환
        const dynamicLinks = data.map(table => ({
          path: `/Control/Board/${table.boardId}`,
  ...
  return (
    <div className="Control" style={{ display: 'flex', width: '100%', height: '100%' }}>
